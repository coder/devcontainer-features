...
# The server is started by the entrypoint and needs a moment to come up.
wait_for_server() {
    for _ in $(seq 30); do
...
check "vscode-web listening" lsof -i "@127.0.0.1:13338"

check "vscode-web log-file" test -f /tmp/vscode-web.log
